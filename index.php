...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $errors['phones'] = !empty($_COOKIE['phone_error']);
...
$values['phones'] = isset($_COOKIE['phone_value']) ? strip_tags($_COOKIE['phone_value']) : '';
...
if (empty($_COOKIE['language_value'])) {
        ...
        $values['languages'] = json_decode($_COOKIE['language_value'], true);  
    }
    ...
    if (!empty($_COOKIE[session_name()]) && !empty($_SESSION['login'])) {
        $db = new PDO('mysql:host=localhost;dbname=u67379', 'u67379', '9399908', array(PDO::ATTR_PERSISTENT => true));
        $stmt = $db->prepare("SELECT * FROM application WHERE id = ?");
        $stmt->execute([$_SESSION['uid']]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        $values['names'] = strip_tags($row['names']);
        $values['phones'] = isset($_COOKIE['phone']) ? strip_tags($_COOKIE['phone']) : '';
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $values['languages'] = isset($_COOKIE['language_value']) ? json_decode($_COOKIE['language_value'], true) : array();
        }
    ...
    if (!preg_match('/^\d+$/', $_POST['phones'])) {
        setcookie('phone_error', '1', time() + 24 * 60 * 60);
        $errors = TRUE;
    } else {
        setcookie('phone_value', $_POST['phones'], time() + 30 * 24 * 60 * 60);
    }
    ...
    if (!empty($_POST['languages'])) {
        $json = json_encode($_POST['languages']);
    ...
    if (!empty($_COOKIE[session_name()]) &&
        ...
        $stmt->execute([$_POST['names'], $_POST['phones'], $_POST['email'], $_POST['dates'], $_POST['gender'], $_POST['biography'], $_SESSION['uid']]);
        $stmt = $db->prepare("DELETE FROM languages WHERE id = ?");
        $stmt->execute([$_SESSION['uid']]);
        $ability = $_POST['languages'];
        ...
        $stmt->execute([$_POST['names'], $_POST['phones'], $_POST['email'], $_POST['dates'], $_POST['gender'], $_POST['biography']]);
